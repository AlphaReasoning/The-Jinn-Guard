...
| RR-004 | Per-agent cryptographic isolation is file-backed HMAC v1, not certificate-bound tenant identity. | Partially mitigated | `agent_nodes[].allowed_peer_uids` binds signed `agent_id` values to `SO_PEERCRED` UIDs; optional `--agent-secret-dir` requires a per-agent HMAC key for selected ids; shared-key rotation supports current/previous keys; startup validates secret dir and file ownership/mode (fails closed on world-readable, world-writable, or group-writable files, or wrong owner UID). | Add certificate-bound or fleet-distributed per-agent identities for mutually distrusting tenants. |
...
| RR-008 | Semantic risk scoring still includes local heuristic classification. | Partially mitigated | Z3 ceilings, intent allowlists, canaries, RootAI remote mTLS fallback, and fail-closed gates bound the classifier's authority. | Replace keyword heuristic with daemon-authoritative semantic scoring once validated. |
