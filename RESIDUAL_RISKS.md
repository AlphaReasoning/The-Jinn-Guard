...
| RR-004 | Per-agent UID binding is implemented, but per-agent cryptographic keys are not. | Partially mitigated | `agent_nodes[].allowed_peer_uids` binds signed `agent_id` values to `SO_PEERCRED` UIDs; HMAC key rotation supports current/previous keys. | Add per-agent secrets or certificate-bound identities for mutually distrusting tenants. |
