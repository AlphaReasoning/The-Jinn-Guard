...
| RR-004 | Per-agent cryptographic isolation is file-backed HMAC v1, not certificate-bound tenant identity. | Mitigated | `agent_nodes[].allowed_peer_uids` binds UDS callers to UIDs; `--agent-secret-dir` provides per-agent HMAC keys; the MCP gateway now extracts X.509 SAN/CN identities when mTLS is enabled. | Provide operator guidance for issuing SPIFFE SVIDs or tenant certs for the MCP gateway. |
